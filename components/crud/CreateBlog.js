...
const signoutUser = () => { signout(); window.open(`${BACKEND}/logout`, '_self'); };
...
const CreateBlog = ({ router }) => {
  ...
  const [values, setValues] = useState({
    ...
    success: '',
    publishText: 'Publish Post',
    formData: typeof window !== 'undefined' ? new FormData() : null,
  });

  const { title, mtitle, mdesc, slug, photo, date, error, success, publishText, formData } = values;
  const token = getCookie('token');
  ...
  useEffect(() => {
    ...
    getCategories().then(data => {
      if (data.error) setValues(v => ({ ...v, error: data.error }));
  ...
  const handleBodyChange = content => {
    ...
    if (typeof window !== 'undefined') localStorage.setItem('blog', JSON.stringify(content));
  };
  ...
  const toggleCategory = cId => {
    const updated = checkedCategories.includes(cId)
      ? checkedCategories.filter(c => c !== cId)
      : [...checkedCategories, cId];

    setCheckedCategories(updated);
    formData.delete('categories');
    updated.forEach(c => formData.append('categories', c));
  };

  // Toggle tags
  const toggleTag = tId => {
    const updated = checkedTags.includes(tId)
      ? checkedTags.filter(t => t !== tId)
      : [...checkedTags, tId];

    setCheckedTags(updated);
    formData.delete('tags');
    updated.forEach(t => formData.append('tags', t));
  };
  ...
  const publishBlog = async e => {
    ...
    formData.set('slug', slugify(slug).toLowerCase());

    try {
      const data = await createBlog(formData, token);
      if (data.error) setValues({ ...values, error: data.error, publishText: 'Publish Post' });
      else {
        setValues({
          title: '',
          mtitle: '',
          ...
          date: null,
          error: '',
          success: `Blog "${data.title}" created successfully!`,
          publishText: 'Publish Post',
        ...
        setCheckedTags([]);
        Router.replace(`/${slugify(slug).toLowerCase()}`);
      }
    } catch (err) {
      setValues({ ...values, error: 'Something went wrong', publishText: 'Publish Post' });
    }
  };

  // UI Helpers
  const showError = () => error && <div className={styles.error}>{error}</div>;
  const showSuccess = () => success && <div className={styles.success}>{success}</div>;

  const showCategories = () => categories.map(c => (
    <li key={c._id} className={styles.listitem}>
      <input type="checkbox" checked={checkedCategories.includes(c._id)} onChange={() => toggleCategory(c._id)} />
      <label>{c.name}</label>
    </li>
  ));

  const showTags = () => tags.map(t => (
    <li key={t._id} className={styles.listitem}>
      <input type="checkbox" checked={checkedTags.includes(t._id)} onChange={() => toggleTag(t._id)} />
      <label>{t.name}</label>
    </li>
  ));
  ...
  const adminTopbar = () => (
    isAuth() && (
      <div className={styles.gridcontainer00}>
        <div className={styles.griditem400}><Image src="/Admin.png" width={18} height={18} alt="Admin" /></div>
        <div className={styles.griditem100}>{isAuth().name}'s Dashboard</div>
        <div className={styles.griditem200} onClick={signoutUser}><Image src="/Logout.png" width={18} height={18} alt="Logout" /> Signout</div>
        <div className={styles.griditem300} onClick={darkmode}><Image src="/WhiteMoon.png" width={16} height={16} alt="Darkmode" /></div>
      </div>
    )
  );
  ...
  return (
    <>
      ...
      <form onSubmit={publishBlog} style={{ background: "var(--adminBack-color)" }}>
        <div>
          <input type="text" placeholder="Title" value={title} className={styles.inputs} onChange={handleChange('title')} required />
        </div>
        <div className={styles.row}>
          ...
          <div className={styles.column2}>
            ...
            <div className={styles.myinputs}>
              <div className={styles.fieldtext}>Published Date</div>
              <DatePicker selected={date} onChange={handleDateChange} minDate={new Date()} dateFormat="dd MMM, yyyy" required />

              <div className={styles.fieldtext}>Meta Title</div>
              <input type="text" value={mtitle} onChange={handleChange('mtitle')} className={styles.inputs2} required />

              <div className={styles.fieldtext}>Meta Description</div>
              <textarea value={mdesc} onChange={handleChange('mdesc')} rows="8" className={styles.inputs2} required />

              <div className={styles.fieldtext}>Slug</div>
              <input type="text" value={slug} onChange={handleChange('slug')} className={styles.inputs2} required />

              <div className={styles.fieldtext}>Featured Image</div>
              <input type="text" value={photo} onChange={handleChange('photo')} className={styles.inputs2} required />
            </div>

            <div className={styles.cat}>
              <div className={styles.heading1}>Categories</div>
            ...
            <button type="submit" className={styles.postbtn}>{publishText}</button>
          </div>
        </div>
      </form>
    </>
